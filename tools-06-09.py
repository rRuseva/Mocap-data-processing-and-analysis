...
def plot_hands_location(start_frame, end_frame, data, mlist):
	...
	y1, c1 = hand_location(start_frame, end_frame, data, mlist, h='R') 
	y2, c2 = hand_location(start_frame, end_frame, data, mlist, h='L') 
...
def hand_speed(start_frame, end_frame, data, mlist, h='R'):
	...
	for frame in range(0, tot_frames-1,1):
		speed_change[frame] =(speed[frame+1]-speed[frame])/2

	return speed, speed_change

def hand_speed_f(start_frame, end_frame, data, mlist, h='R'):
	tot_frames = end_frame - start_frame
	temp_data = data[start_frame:end_frame+1, :, :]
	speed = np.zeros([tot_frames, 3], dtype = float)
	speed_change = np.zeros([tot_frames, 3], dtype = float)
...
def plot_hand1(start_frame, end_frame, data, mlist, fps):
	...
	# #lenght of 
	# tot-frames = end_frame - start_frame
	# fs = 
...
def plot_hand2(start_frame, end_frame, data, mlist, fps):
	r_hand_tr, r_arm_tr = hand_trajectory(start_frame, end_frame, data, mlist, 'R')
	...
	plt.subplot(4, 2, 3)
	plt.plot(x, r_vel_norm, 'r-', label="Original")
	...
	plt.subplot(5, 1, 4)
	plt.plot(x, r_acc_norm, 'r-', label="Original")
	plt.plot(x, r_hand_acc_filt, 'g-', label="Filtered")

	plt.plot(x, r_hand_acc_filt1, 'b-', label="Filtered")
	plt.plot(x, r_hand_acc_filt2, 'y-', label="Filtered")
	plt.ylabel("Acceleration (mm/frame^2)") 
	plt.grid(True)
...
def segm(acc, acc_ex, start_frame, end_frame, data, mlist, h='R'):
...
def segment_signs(start_frame, end_frame, data, mlist, fps):
	...
	plt.title("{} signs between {} and {} frame".format(count, start_frame, end_frame))
