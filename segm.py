...
if __name__ == "__main__":
	...
	# st = 1729
	# en = 1998
	# diff_right_hand, diff_left_hand, diff_RWRE, diff_LWRE, right_dominant, one_hand = t.hand_displacment(st, en, new_data, marker_list)
	# print("*****")
	# # print(sign[0])
	# message = "The sign between {} - {} frames is".format(st, en)
	# if(one_hand == 3):
	...
	# # extr = t.find_extremums(acc_f)
	# zeros = t.interesting_points(acc_f)
	# # print(zeros)
	...
	# print('-Real start 1 and end 1')
	# print("{}-{}".format(start1+st, end1+st))
	# # ss, ss1 = t.get_signs_borders(l, start_frame, end_frame)
	# # print(ss.shape)
	# # c = len(ss)
	# print()
	...
	# fig1 = plt.figure("{}-{}-{}sign-vel".format(title,st,en), figsize=(10.5,7))
	# fig1.suptitle("Hand velocity for sign between {} and {} frame".format(st, en))	

	# plt.plot(x, vel_norm, 'c', label='Normilized velocity') 
	# plt.plot(x, acc, 'm', label='Filtered acc') 
	# plt.plot(x[zeros], vel_norm[zeros], 'o')
	# # plt.plot(x[extr], vel_norm[extr], 'mo')
	...
	# # plt.plot(x[ss[:, 0]], r_vel[ss[:, 0]], 'rs', label = "Start")	
	# # plt.plot(x[ss[:, 1]], r_vel[ss[:, 1]], 'r*', label = "End")	
	...
	for sign in enumerate(signs2):
		...
		print("*****")
		print(sign[0]+1)
		message = "{} The sign between {} - {} frames is".format(sign[0]+1, st, en)
		if(one_hand == 3):
			message = message + ' two handed'
		elif(one_hand == 1):
			message = message + ' right handed'
